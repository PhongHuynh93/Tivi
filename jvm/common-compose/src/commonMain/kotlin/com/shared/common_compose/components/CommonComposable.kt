...
@Composable
fun ConnectionStatus(isConnected: Boolean, additionalPaddingModifier: Modifier) {
    val backgroundColor by animateColorAsState(if (isConnected) green else colorError)
    val message = if (isConnected) {
        stringResource(MR.strings.status_connected)
    } else {
        stringResource(MR.strings.status_no_connection)
    }
    ...
    Box(
        modifier = Modifier
            ...
            .then(additionalPaddingModifier)
            .fillMaxWidth()
            .padding(10.dp),
